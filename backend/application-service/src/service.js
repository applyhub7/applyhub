...
export async function applyToJob(user, body) {
  if (user.role !== "candidate") return { error: { status: 403, message: "forbidden" } };
  if (!body.jobId || !body.jobTitle || !body.companyId || !body.candidateEmail || !body.resumeData || !body.resumeFileName) {
  ...
  try {
    ...
    return { error: { status: 500, message: error?.message || "failed to submit application" } };
  }
}
...
export async function changeApplicationStatus(user, id, status) {
  if (user.role !== "recruiter") return { error: { status: 403, message: "forbidden" } };
  if (!allowedStatuses.includes(status)) return { error: { status: 400, message: "invalid status" } };
  const application = await findApplicationById(id);
  if (!application) return { error: { status: 404, message: "application not found" } };
  if (application.company_id !== user.id) return { error: { status: 403, message: "forbidden" } };
  if (!canTransition(application.status, status) && application.status !== status) {
    return { error: { status: 400, message: "invalid transition" } };
  }
  return { application: await updateApplicationStatus(id, status) };
}
