...
export async function gatewayRoutes(app) {
  app.get("/health", async () => ({ ok: true, service: "gateway", environment: gatewayConfig.nodeEnv }));
  ...
  app.all("/jobs/*", async (request, reply) => forward(request, reply, gatewayConfig.jobUrl));
  app.all("/applications", async (request, reply) => forward(request, reply, gatewayConfig.applicationUrl));
  app.all("/applications/*", async (request, reply) => forward(request, reply, gatewayConfig.applicationUrl));
}
